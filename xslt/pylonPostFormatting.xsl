...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:hei="https://digi.ub.uni-heidelberg.de/schema/tei/heiEDITIONS" xpath-default-namespace="http://www.tei-c.org/ns/1.0" xmlns="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="hei" version="3.0">
  ...
  -->
                ...
                <xsl:for-each select="TEI/TEI[descendant::idno[@type = 'HGV']][descendant::ab]">
                    <relatedItem type="mentions">
                        ...
                        <bibl>
                            ...
                            <biblScope type="vol">
                                <xsl:number value="./descendant::idno[@type = 'ddb-hybrid']/(tokenize(., ';'))[2]" format="I"/>
                            </biblScope>
                ...
                <teiHeader>
                    ...
                    <revisionDesc>
                        <change who="https://papyri.info/editor/users/HGV">
                            <xsl:attribute name="when"><xsl:value-of select="current-date()"/></xsl:attribute>Xwalk from Pylon</change>
                    </revisionDesc>
                </teiHeader>
                <text>
                    <body>
    ...
    <xsl:template match="div">
        <xsl:element name="div">
            <xsl:apply-templates select="@*[name() != 'xml:id' and name() != 'part' and name() != 'org' and name() != 'sample']"/>
    ...
    <xsl:template match="*">
        <xsl:element name="{local-name()}" xmlns="http://www.tei-c.org/ns/1.0" xmlns:hei="https://digi.ub.uni-heidelberg.de/schema/tei/heiEDITIONS">
            <xsl:copy-of select="@*[not(name() = 'instant' or name() = 'full' or name() = 'part' or name() = 'default' or name() = 'sample' or name() = 'org')]"/>
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
    ...
    <!--
    ================
    replacing | with <lb n="\d+" break="no"/>
